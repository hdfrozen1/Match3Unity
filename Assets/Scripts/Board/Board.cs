...
public class Board
{
    ...
    {
        // Kiểm tra toàn bộ mảng m_cells đã khởi tạo ở CreateBoard()
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                {
                    return false; // Còn ít nhất 1 ô có Item
                }
            }
        }
        return true; // Tất cả đều trống
    }
    ...
    internal void Fill()
    {
        ...
        int groupsOfThree = totalCells / 3;
        List<NormalItem.eNormalType> typePool = new List<NormalItem.eNormalType>();

        // 1. Lấy danh sách tất cả các loại Item hiện có
        var allTypes = (NormalItem.eNormalType[])System.Enum.GetValues(typeof(NormalItem.eNormalType));
        int numUniqueTypes = allTypes.Length;

        // 2. BƯỚC 1: Ưu tiên chọn mỗi loại ít nhất một bộ 3
        // Lưu ý: Nếu số lượng bộ 3 mà bảng chứa được ít hơn số loại hiện có, 
        // chúng ta chỉ lấy được tối đa 'groupsOfThree' loại đầu tiên.
        int typesToIncludeFirst = Mathf.Min(numUniqueTypes, groupsOfThree);

        for (int i = 0; i < typesToIncludeFirst; i++)
        {
            typePool.Add(allTypes[i]);
            typePool.Add(allTypes[i]);
            typePool.Add(allTypes[i]);
        }

        // 3. BƯỚC 2: Nếu vẫn còn dư chỗ cho các bộ 3 khác, chọn ngẫu nhiên để lấp đầy
        int remainingGroups = groupsOfThree - typesToIncludeFirst;
        for (int i = 0; i < remainingGroups; i++)
        {
            NormalItem.eNormalType randomType = allTypes[UnityEngine.Random.Range(0, allTypes.Length)];
            typePool.Add(randomType);
            typePool.Add(randomType);
            typePool.Add(randomType);
        }

        // 4. Xử lý trường hợp đặc biệt: Nếu tổng số ô không chia hết cho 3
        // (Thường nên tránh điều này bằng cách đặt BoardSizeX * BoardSizeY là bội số của 3)
        while (typePool.Count < totalCells)
        {
            typePool.Add(allTypes[UnityEngine.Random.Range(0, allTypes.Length)]);
        }

        // 5. Xáo trộn (Shuffle) danh sách để các loại rải đều khắp bàn
        for (int i = 0; i < typePool.Count; i++)
        {
            int rnd = UnityEngine.Random.Range(i, typePool.Count);
            var temp = typePool[i];
            typePool[i] = typePool[rnd];
            typePool[rnd] = temp;
        }

        // 6. Đổ Item vào các ô trên Board
        int poolIndex = 0;
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                {
                    Item item = CreateItemWithType(typePool[poolIndex]);
                    m_cells[x, y].Assign(item);
                    m_cells[x, y].ApplyItemPosition(true);
                    poolIndex++;
                }
            }
        }
    ...
    // Đảm bảo bạn có hàm này để tạo Item theo đúng loại chỉ định
    private Item CreateItemWithType(NormalItem.eNormalType type) // ham dat tao
    {
        NormalItem item = new NormalItem();
        item.SetType(type);
        item.SetView();
