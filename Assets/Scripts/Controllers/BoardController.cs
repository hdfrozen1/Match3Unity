...
public class BoardController : MonoBehaviour
{
    ...
    public void StartGame(GameManager gameManager, GameSettings gameSettings, TrayManager trayManager)
    {
    ...
    public void Update()
    {
        ...
        if (IsBusy) return;
        if (m_trayManager.IsBusy) return;
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                Debug.Log(clickedCell.gameObject.name);
                Debug.Log(clickedCell.Item);
                ...
                // 2. Check xem có bấm vào Tray không (dùng hàm mới)
                if (m_gameManager.CurrentMode == GameManager.eLevelMode.TIMER)
                {
                    Item itemInTray = m_trayManager.GetItemFromHit(clickedCell);
                    if (itemInTray != null)
                    {
                        HandleReturnToBoard(itemInTray);
                    }
                }
            }
        }
    ...
    private void HandleReturnToBoard(Item item)
    {
        // Kiểm tra xem ô gốc của nó có đang trống hay không
        ...
        if (targetCell != null)
        {
            ...
            // Xóa khỏi danh sách quản lý của khay
            m_trayManager.RemoveItem(item);

            // Gán lại vào ô trên bàn
            targetCell.Assign(item);

            // Hiệu ứng bay ngược về vị trí cũ
            item.View.DOMove(targetCell.transform.position, 0.3f).OnComplete(() =>
            {
                IsBusy = false;
                // (Tùy chọn) Reset Sorting Layer về bình thường nếu cần
                item.SetSortingLayerLower();
            });
        }
    }
    private void HandleClick(Cell clickedCell)
    {
        if (m_trayManager.CanAddItem())
        {
            IsBusy = true;
            Item item = clickedCell.Item;

            // BƯỚC 1: Lưu lại ô này là ô gốc
            item.OriginalCell = clickedCell;
            Debug.Log(clickedCell.gameObject.name);

            // BƯỚC 2: Giải phóng ô trên Board
            clickedCell.Free();

            // BƯỚC 3: Đưa vào khay
            m_trayManager.AddItem(item, () =>
            {
                if (m_board.IsBoardEmpty()) m_gameManager.GameWin();
                IsBusy = false;
            });
        }
    }
